...
void Particle::setup(){
    ...
    //どれくらい動いたらパーティクルを発生させるかの閾値
    thresh = 1.0;
};

void Particle::setCenter(float x, float y){
    
    //前回のxy座標と今回のxy座標でベクトルを作る
    ofVec2f vec = prePos - ofVec2f(x,y);
    
    //十分座標が動いていたら発生(ベクトルの長さを見る)
    if(vec.length() > thresh){
        
        // 毎回ランダムな数のパーティクルが発生
        int rep = ofRandom(1, 4);
        ...
        for(int i = 0; i < rep; i++){
            ...
            //色をランダムに
            ofFloatColor color;
            color.setHsb(ofRandom(0.0, 0.25), 1.0, 1.0);
...
void Particle::setFromContourFinder(ofxCvContourFinder &contourFinder){
   
    // contourFinderが検出したblobの数だけ繰り替えし処理
    for (int i = 0; i < contourFinder.nBlobs; i++){
        
        // 輪郭線の最小xyと最大xyから出来る四角形がboundingRect
        // その四角形の中心のxy座標を検出
        float x = contourFinder.blobs[i].boundingRect.getCenter().x;
        float y = contourFinder.blobs[i].boundingRect.getCenter().y;
...
void Particle::update(){
    for(int i = 0; i < NUM_PARTICLES; i++){
        ...
        colors[i].a -= 0.05; //パーティクルを少しずつ透明に
    }
